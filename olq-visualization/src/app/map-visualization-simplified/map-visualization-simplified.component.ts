...
import { circle, DomEvent, latLng, Map, marker, tileLayer, point, icon } from 'leaflet';
import { BestLocationService } from '../best-location-service';
import { Place } from '../map-visualization/Place';

@Component({
  selector: 'app-map-visualization-simplified',
  templateUrl: './map-visualization-simplified.component.html',
  styleUrls: ['./map-visualization-simplified.component.css'],
  providers: [BestLocationService]
})
export class MapVisualizationSimplifiedComponent implements OnInit {
  ...
  constructor(private bestLocationService: BestLocationService) {
    ...
    this.googleMaps = tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      maxZoom: 20,
  ...
  private putCandidatesOnMap(candidates) {
    for (const candidate of candidates) {
      ...
      const bestLocationMarker = marker([candidate.latitude, candidate.longitude], {
        icon: candidateIcon,
      });

      bestLocationMarker.addTo(this.map);
